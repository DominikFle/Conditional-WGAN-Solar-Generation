  ...
  for i,hour in enumerate(series):
    if hour == 23:
      ...
      if len_hour_series -1  == i:
        break
      assert (hour+1)==series[i+1], f"at index {i} the val {hour} is followed by {series[i+1]}"
...
def drop_days_with_gaps(hour_series:pd.Series, additional_drops:list[pd.Series], verbose=True):
  """
  ...
  for i,hour in enumerate(hour_series):
    ...
    for series in additional_drops:
